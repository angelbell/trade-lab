...
Run (full):    .venv/bin/python scratchpad/btc15mL_pivot_exit.py 2>&1 | tee scratchpad/out_btc15mL_pivot_exit.txt
Run (smoke):   .venv/bin/python scratchpad/btc15mL_pivot_exit.py --smoke 2>&1 | tee scratchpad/out_btc15mL_pivot_exit_smoke.txt
"""
import argparse
import contextlib
import io
...
def cdd(vals, days):
    ...
    scratchpad/book_leave_one_out.py -- do not re-derive)."""
...
def build_book_legs(gold_start="2018-01-01", gold15m_skip=False, s_rr=4.5, btc15mL_override=None):
    """Reuses scratchpad/book_spec_fix.build() VERBATIM (the M1+M2 spec-fixed 6-leg
...
def book_monthly(legs, basket=NEW_LEGS, budget=0.03):
    """Reused machinery from scratchpad/book_bootstrap_arbiter.py: the book's MONTHLY
...
def block_bootstrap(monthly_a, monthly_b, ndraw=2000, seed=20260713):
    ...
    scratchpad/book_bootstrap_arbiter.py. Returns {block: (median_cagr_dd_b, P(b>a))}."""
