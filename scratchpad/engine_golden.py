"""Golden snapshot for the engine delegation (Phase 3).
  ...
  dump : run the CURRENT breakout_wave.run / ema_pullback.run on the canonical legs
         and pickle the trade tables + stdout -> scratchpad/engine_golden.pkl
  check: run src.engine run_compat / run_ema_compat (or the delegated run()) and
         require exact equality with the pickled goldens.

Usage:
  .venv/bin/python scratchpad/engine_golden.py dump
  .venv/bin/python scratchpad/engine_golden.py check        # engine vs golden
  .venv/bin/python scratchpad/engine_golden.py check-run    # breakout_wave.run vs golden (post-delegation)
"""
...
PKL = f"{ROOT}/scratchpad/engine_golden.pkl"
