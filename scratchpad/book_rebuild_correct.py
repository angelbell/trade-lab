...
scratchpad/book_leave_one_out.py's own leave-one-out only fixed bug 1 -- it still calls its
...
New code in this file (the actual experiments, not a re-derivation of the arbiter):
  ...
  - R2: leave-one-out, weights re-derived over the surviving 5 legs each time; bootstrap;
    ...
    session (scratchpad/out_book_leave_one_out_REFERENCE_prevrun.txt) for comparison.
...
Run (smoke, fewer bootstrap draws): .venv/bin/python scratchpad/book_rebuild_correct.py --smoke
Run (full):                          .venv/bin/python scratchpad/book_rebuild_correct.py
"""
...
# session (scratchpad/out_book_weighting_scheme_smoke_CHECK.txt), E2's invvol_trade row.
...
# (scratchpad/out_book_leave_one_out_REFERENCE_prevrun.txt). Not recomputed here.
OLD_LOO_CD = {
