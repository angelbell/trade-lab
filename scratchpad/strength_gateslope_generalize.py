"""仕様カード scratchpad/spec_gateslope_generalize.md の実装。

問い: btc15m_L で「ゲート指標(4H KAMA)の傾きの急さ」が本物の強度勾配だった(カード2)。
これが他レッグでも成り立つ横断プリミティブかを、gold15m と btc15m_S(ショート鏡像)で検定する。

土台は scratchpad/strength_btc15mL.py (base) と strength_regime_btc15mL.py (reg) をそのまま
...
Run:
  .venv/bin/python scratchpad/strength_gateslope_generalize.py --smoke 2>&1 | tee scratchpad/out_strength_gateslope_generalize_smoke.txt
  .venv/bin/python scratchpad/strength_gateslope_generalize.py 2>&1 | tee scratchpad/out_strength_gateslope_generalize.txt
"""
import argparse
import contextlib
import io
...
def main():
    ...
    print(f"\n実行コマンド: .venv/bin/python scratchpad/strength_gateslope_generalize.py"
