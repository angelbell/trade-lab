...
流用（車輪の再発明禁止）:
  ...
  - scratchpad/A_daily_regime.py の MFE ループ定義 … 素の巡行幅（stop-only, fwd=500, rr=100）
  - scratchpad/arb_common.Boot                    … 巡回ブロック・ブートストラップの層(月ブロック抽選)
...
Run:
  .venv/bin/python scratchpad/M_squeeze_screen.py --smoke   # 直近データで先に通す
  .venv/bin/python scratchpad/M_squeeze_screen.py           # フル
"""
