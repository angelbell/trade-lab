"""仕様カード scratchpad/spec_gold15m_fast_accel.md の実装。
...
土台(再発明禁止):
  - gold15m のレッグ再構築(entries直呼び＋トレード<->確定足i対応、照合ゲート1/2/3) は
    scratchpad/strength_gateslope_generalize.py の build_gold15m/gate1_check/gate2_check/
  ...
  - quintile_table/monotone_flag/block_bootstrap_spearman/random_drop_null/report_candidate は
    scratchpad/strength_btc15mL.py の汎用実装をそのまま import。
  - h4_kama_slope は scratchpad/strength_regime_btc15mL.py の compute_kama_slope を
...
Run:
  .venv/bin/python scratchpad/strength_gold15m_fastaccel.py --smoke 2>&1 | tee scratchpad/out_strength_gold15m_fastaccel_smoke.txt
  .venv/bin/python scratchpad/strength_gold15m_fastaccel.py 2>&1 | tee scratchpad/out_strength_gold15m_fastaccel.txt
"""
import argparse
import os
import sys
...
def main():
    ...
    print(f"\n実行コマンド: .venv/bin/python scratchpad/strength_gold15m_fastaccel.py"
