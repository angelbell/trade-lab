...
Reuses (imports, does not reimplement):
  - experiments/btc_family_ext_throttle.py: build_base()      -- the 6 canonical leg R-series
  - experiments/book_leave_one_out.py: cdd(), weights(), stat(), NEW, OLD
  - experiments/book_weighting_scheme.py: monthly_matrix(), scheme_invvol_trade(),
      series_from_weights()                                   -- the specified book weighting
  - experiments/book_hh4h_weight_sweep.py: leg_stats()          -- standardized single-leg
...
Run (full):  .venv/bin/python experiments/btc15mL_regime_concentration.py 2>/dev/null | tee experiments/out_btc15mL_regime_concentration.txt
Run (smoke): .venv/bin/python experiments/btc15mL_regime_concentration.py --smoke 2>/dev/null | tee experiments/out_btc15mL_regime_concentration_smoke.txt
"""
