...
experiments/context_time_fomc.py の et14_to_utc_broker と同じ手法(ET固定時刻->tz_localize->
tz_convert(UTC/Europe-Riga))を 08:30 ET に置き換えて流用する(手法自体は同スクリプトの
validate_tz_method で既存48行のFOMC日付に対して再検算し、完全一致することを確認する)。

第二部: experiments/event_scalp_cond.py / event_scalp.py の関数を無改変でimportし、
...
実行:
  .venv/bin/python experiments/news_scalp_combined.py --smoke 2>&1 | tee experiments/out_news_scalp_combined_smoke.txt
  .venv/bin/python experiments/news_scalp_combined.py 2>&1 | tee experiments/out_news_scalp_combined.txt
"""
import argparse
import os
import sys
...
#    使うときは環境変数で渡す:  FRED_API_KEY=xxxx .venv/bin/python experiments/news_scalp_combined.py
...
def main():
    ...
    print(f"\n\n{'='*100}\nDONE\n{'='*100}")
    print(f"\n実行コマンド: .venv/bin/python experiments/news_scalp_combined.py{' --smoke' if args.smoke else ''}")
