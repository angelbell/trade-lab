"""仕様カード2 experiments/spec_strength_regime_btc15mL.md の実装。

btc15m_L を experiments/strength_btc15mL.py の土台（entries直呼び再構築＋トレード<->確定足i
...
Run:
  .venv/bin/python experiments/strength_regime_btc15mL.py --smoke 2>&1 | tee experiments/out_strength_regime_btc15mL_smoke.txt
  .venv/bin/python experiments/strength_regime_btc15mL.py 2>&1 | tee experiments/out_strength_regime_btc15mL.txt
"""
import argparse
import contextlib
import io
...
def main():
    ...
    print(f"\n実行コマンド: .venv/bin/python experiments/strength_regime_btc15mL.py"
