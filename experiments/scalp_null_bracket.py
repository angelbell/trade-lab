...
Usage:
  .venv/bin/python experiments/scalp_null_bracket.py --smoke
  .venv/bin/python experiments/scalp_null_bracket.py 2>&1 | tee experiments/out_scalp_null_bracket.txt
"""
...
def run(args):
    ...
    main_df.to_csv("experiments/scalp_null_bracket_main.csv", index=False)
    if yearly_rows:
        pd.DataFrame(yearly_rows).to_csv("experiments/scalp_null_bracket_yearly.csv", index=False)
