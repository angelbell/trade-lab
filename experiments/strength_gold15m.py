"""仕様カード7 experiments/spec_strength_gold15m.md の実装。
...
土台（再発明禁止・import流用）:
  experiments/strength_gateslope_generalize.py -- build_gold15m()（照合ゲート1-3 済みの
                                          gold15m 構築経路。netR定義もbook.pyと一致確認済み）・
                                          compute_sma_slope（日足SMA150傾き, combo3用）
  experiments/strength_btc15mL.py       -- rebuild_entries/match_entries_to_trades/
  ...
  experiments/strength_transplant_btc.py -- build_combo/topgap_table/block_bootstrap_topgap/
...
Run:
  .venv/bin/python experiments/strength_gold15m.py --smoke 2>&1 | \\
      tee experiments/out_strength_gold15m_smoke.txt
  .venv/bin/python experiments/strength_gold15m.py 2>&1 | \\
      tee experiments/out_strength_gold15m.txt
"""
import argparse
import os
import sys
...
def main():
    ...
    print(f"\n実行コマンド: .venv/bin/python experiments/strength_gold15m.py"
