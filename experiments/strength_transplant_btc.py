"""仕様カード6 experiments/spec_strength_transplant_btc.md の実装。
...
土台（再発明禁止・import流用）:
  experiments/strength_btc15mL.py       -- build/rebuild_entries/match_entries_to_trades/
                                           quintile_table/block_bootstrap_spearman/report_candidate
  experiments/strength_gateslope_generalize.py -- btc15m_S の構築(build_btc15mS)・
                                           gate1_check/gate2_check（照合ゲート済みの経路）
  experiments/strength_entryquality_btc15mL.py -- atr_percentile_at（trailing500分位）
  experiments/strength_regime_btc15mL.py       -- compute_kama_slope（4H/日足KAMA傾き、
...
Run:
  .venv/bin/python experiments/strength_transplant_btc.py --smoke 2>&1 | \\
      tee experiments/out_strength_transplant_btc_smoke.txt
  .venv/bin/python experiments/strength_transplant_btc.py 2>&1 | \\
      tee experiments/out_strength_transplant_btc.txt
"""
import argparse
import contextlib
import io
...
def main():
    ...
    print(f"\n実行コマンド: .venv/bin/python experiments/strength_transplant_btc.py"
