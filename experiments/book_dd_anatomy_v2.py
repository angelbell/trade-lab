...
Frozen premises (not re-derived, imported):
  - experiments/btc_family_ext_throttle.py: build_base() -- 6 canonical legs (gold_bo, btc_bo_kama,
  ...
  - experiments/book_arbiter_v2.py: book_monthly_gen() (inv-vol/3%-budget weights, UNCHANGED) and
    ...
    was written).
  - experiments/book_hh4h_weight_sweep.py: ROOT, OLD (3-leg), NEW (6-leg) basket lists.
...
Run (smoke -- tie-back only, stops before Steps 1-3):
  .venv/bin/python experiments/book_dd_anatomy_v2.py --smoke
Run (full):
  .venv/bin/python experiments/book_dd_anatomy_v2.py
"""
