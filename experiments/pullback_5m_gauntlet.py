...
Faithful port of experiments/pullback_fixedtgt.py logic (exact canon reproduction verified).
...
print("\n==== B. PULLBACK-LIMIT rescue @ $0.6 (frac sweep + structural anchor = broken-level retest) ====")
FRACS = (0.2, 0.25, 0.3, 0.38, 0.5)
print(HDR)
res5 = {}
for f in FRACS:
    tr, ms = eval_pull(B5r4, lambda e, s, H, f=f: e - f * (e - s), 0.6)
    res5[f"frac{f}"] = tr; row(f"5m frac{f}", tr, span5, ms)
trH, msH = eval_pull(B5r4, lambda e, s, H: H, 0.6)
res5["H1lvl"] = trH
...
for f in (0.25, 0.3):
    tr, ms = eval_pull(B15r4, lambda e, s, H, f=f: e - f * (e - s), 0.6)
    row(f"15m frac{f}", tr, span15, ms)
tr, ms = eval_pull(B15r4, lambda e, s, H: H, 0.6)
...
print("\n==== D. STOP-SLIP sensitivity (loss = -1 - 0.27*ATR(entry)/risk) @ $0.6 ====")
print(HDR)
row("5m mkt +slip", eval_market(B5r4, 0.6, stop_slip=0.27), span5)
tr, ms = eval_pull(B5r4, lambda e, s, H: e - 0.25 * (e - s), 0.6, stop_slip=0.27)
row("5m f.25 +slip", tr, span5, ms)
tr, ms = eval_pull(B5r4, lambda e, s, H: H, 0.6, stop_slip=0.27)
...
peryear(eval_market(B5r4, 0.6), "5m market")
peryear(res5["frac0.25"], "5m frac0.25")
peryear(res5["H1lvl"], "5m lvl-rt")

print("\n==== F. BETA NULL: same-frequency random long @ $0.6 (200 trials) ====")
...
print("  5m market:"); beta_null(B5r4, eval_market(B5r4, 0.6), 4.0, 0.6)
print("  5m frac0.25:"); beta_null(B5r4, res5["frac0.25"], 4.0, 0.6)
print("  5m lvl-retest:"); beta_null(B5r4, res5["H1lvl"], 4.0, 0.6)

print("\n==== G. HOUR-OF-DAY (server clock) meanR(n), 5m market @ $0.6 + drop 9-15 effect ====")
tr = eval_market(B5r4, 0.6); R = np.array([r for _, r in tr]); hh = np.array([t.hour for t, _ in tr])
...
tr15, _ = eval_pull(B15r4, lambda e, s, H: e - 0.25 * (e - s), 0.6)
