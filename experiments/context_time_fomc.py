"""仕様カード experiments/spec_context_time_fomc.md の実装。
...
対象レッグ(照合済み再構築を流用・車輪の再発明禁止):
  btc15m_A: experiments/strength_btc15mA.py の build_A() (照合ゲート1-4 PASS 済み,
  ...
  gold15m : experiments/strength_gateslope_generalize.py の build_gold15m() (照合ゲート
...
Run:
  .venv/bin/python experiments/context_time_fomc.py --smoke 2>&1 | tee experiments/out_context_time_fomc_smoke.txt
  .venv/bin/python experiments/context_time_fomc.py 2>&1 | tee experiments/out_context_time_fomc.txt
"""
import argparse
import os
import sys
...
def main():
    ...
    if cli.smoke:
        ...
        print(f"\n実行コマンド: .venv/bin/python experiments/context_time_fomc.py --smoke")
        return
    ...
    print("  [注記] n(72/121)が既存記録(41/106)と一致しない -- 母集団の総数自体が違う"
          ...
          "experiments/book_spec_fix.py(2026-07-13)の gold15m 構築には fill_win=200 が"
    ...
    print(f"\n実行コマンド: .venv/bin/python experiments/context_time_fomc.py")
