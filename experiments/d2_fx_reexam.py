...
exactly as experiments/short_mirror_15m.py. Cost: net round-trip 0.9 pips, subtracted per-trade via
R_net = R - rt_cost/risk (rt_cost in price units: 0.0001*0.9 non-JPY, 0.01*0.9 JPY pairs).

Run: .venv/bin/python experiments/d2_fx_reexam.py            (full grid)
     .venv/bin/python experiments/d2_fx_reexam.py --smoke     (1 pair x 1 tf x 1 side)
Output tee'd to experiments/out_d2_fx.txt by the caller.
"""
...
def density_guard_m15(d, min_bars_per_day=60):
    ...
    30-day median bar-count clears the threshold (mirrors experiments/short_mirror_15m.py)."""
