...
Run:
  .venv/bin/python experiments/atr_spike_btc_h1_gates.py --smoke 2>&1 | tee experiments/out_atr_spike_gates_smoke.txt
  .venv/bin/python experiments/atr_spike_btc_h1_gates.py 2>&1 | tee experiments/out_atr_spike_gates.txt
"""
...
def main():
    ...
    gdf.to_csv(os.path.join(ROOT, "experiments/atr_spike_btc_h1_gates_grid.csv"), index=False)
    print(f"\n全格子を保存: experiments/atr_spike_btc_h1_gates_grid.csv ({len(gdf)}行)")
