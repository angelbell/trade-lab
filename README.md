...
|---|---|---|---|---|---|---|---|
| +33.9% | +67.8% | +24.1% | **+26.4%** | +46.0% | +75.2% | **+56.8%** | +22.9% |
...
|---|---|---|---|---|---|---|
| gold_bo | 0.551% | 29 | 41.0% | 1.74 | +0.492 | 7 |
| btc_bo_kama | 0.721% | 8 | **52.6%** | 2.16 | +0.549 | **3** |
| btc_pull | 0.554% | 10 | 47.8% | 2.57 | +0.820 | 4 |
| gold15m | 0.379% | 44 | 25.6% | 1.74 | +0.585 | 11 |
| **btc15m_L** | 0.475% | **100** | 22.8% | 1.86 | +0.445 | **17** |
| btc15m_S | 0.320% | 13 | 33.3% | 2.42 | +1.013 | 7 |
...
|---|---|---|---|---|
| （6レッグ全部） | +63.5% | 7.49% | **8.48** | — |
| **btc15m_L** | +40.5% | 10.50% | **3.85** | **−4.62** |
| btc15m_S | +65.4% | 9.74% | 6.71 | −1.77 |
| gold15m | +58.7% | 7.94% | 7.40 | −1.08 |
| gold_bo | +66.0% | 8.61% | 7.67 | −0.81 |
| btc_pull | +72.6% | 9.16% | 7.93 | −0.55 |
| btc_bo_kama | +82.0% | 9.67% | **8.48** | **±0.00** |

- **`btc15m_L` がブックの生命線。** 抜くと CAGR/DD が **8.48 → 3.85** に落ちる。頻度（年100本）が統計を効かせている。
- **`btc_bo_kama` は CAGR/DD を1ミリも上げないが、DD を下げる。** 抜いても CAGR/DD は同じ 8.48 だが、
  DD は 7.49% → 9.67% に悪化する。5レッグの総リスクを 2.30% に絞って**同じ DD (7.49%) に揃える**と CAGR は 58.9%。
  6レッグは同じ DD で **63.5% ＝ +4.6 ポイント高い。リスク・ダイヤルでは代替できない、本物の分散。**
  機構＝ブックで唯一 勝率>50%・最長連敗3回・R のばらつき最小・
  **ブックの最悪 5% の日に建てている率が 22%**（gold_bo は 55%）＝**そもそも悪い日に市場にいない**。
- **`btc15m_S` は弱気年の中和役。** 2022年は L が −6.6R、S が +20.9R。単体は年13本の小さなレッグだが、抜くと −1.77。
...
- **想定実ドローダウンは 11〜15%**（backtest の 7.49% × 1.5〜2）。バックテストの DD をそのまま信じない。
